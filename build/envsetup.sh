...
function streakremote()
{
    ...
    local STREAK_USER=$(git config --get review.review.projectstreak.pl.username)
    if [ -z "$STREAK_USER" ]
    then
        git remote add streak ssh://review.projectstreak.pl:29418/$PFX$PROJECT
    else
        git remote add streak ssh://$STREAK_USER@review.projectstreak.pl:29418/$PFX$PROJECT
    fi
...
function streakgerrit() {
    ...
    local user=`git config --get review.review.projectstreak.pl.username`
...
function streakrebase() {
    ...
    git fetch "http://review.projectstreak.pl/p/$repo" "refs/changes/$refs" && git cherry-pick FETCH_HEAD
    if [ "$?" != "0" ]; then
